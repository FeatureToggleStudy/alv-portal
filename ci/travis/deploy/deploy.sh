...
DEPLOY_SOURCE_DIR=online-services-deployment/target/docker-swarm-dist
...
ssh -i $DEPLOY_KEY_DECRYPTED -o "StrictHostKeyChecking no" $DEPLOY_USER@$DEPLOY_HOST "chmod +x $DEPLOY_DEST_DIR/scripts/*.sh"
ssh -i $DEPLOY_KEY_DECRYPTED -o "StrictHostKeyChecking no" $DEPLOY_USER@$DEPLOY_HOST "cd $DEPLOY_DEST_DIR/scripts && ./deploy-docker-stack.sh $ENV_SLUG"
