...
export class MultiTypeaheadComponent extends AbstractInput {
  ...
  selectFreeText(): MultiTypeaheadItem<any> {
    if (this.itemLimitReached() || !this.editable || !this.inputValue || this.inputValue.length < TYPEAHEAD_QUERY_MIN_LENGTH) {
      return null;
    }
    const freeTextItem = new SimpleMultiTypeaheadItem('free-text', this.inputValue, this.inputValue);
